...
const ownFilesMiddleware = async (req:any, res:any, next:any) => {
    const sql = "SELECT * FROM files LEFT JOIN lectures ON files.lecture_id = lectures.id LEFT JOIN groups ON lectures.group_id = groups.id LEFT JOIN courses ON groups.course_id = courses.id LEFT JOIN users ON courses.lecturer_id = users.id WHERE (users.id = ? AND lectures.id =?)";
